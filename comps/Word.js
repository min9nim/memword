...
const remove = async (word, dom) => {
  ...
  if (json.status === 'Fail') {
    ...
    app.view.Index.setState({
      list: app.view.Index.state.list.filter(o => o.id !== word.id),
    })
  }
}
    ...
    return (
      ...
      >
        <div className="word">
          ...
          <div className="date">{moment(word.updatedAt).fromNow()}</div>

          <div className="name">{`by ${word.userName}`}</div>

          <div
            className="remove"
            onClick={() => remove(word, this.dom)}
            onMouseLeave={() => mouseleave(this.dom)}
            onMouseEnter={() => mouseenter(this.dom)}
          >
